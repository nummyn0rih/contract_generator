...
with st.sidebar:
    st.header("Настройки")
    api_key_present = bool(os.getenv("GIGACHAT_CREDENTIALS"))
    if api_key_present:
        st.success("GIGACHAT_CREDENTIALS загружен")
    else:
        st.error("GIGACHAT_CREDENTIALS не найден. Создайте .env по образцу .env.example.")
    st.write(f"**Модель:** `{os.getenv('GIGACHAT_MODEL', 'GigaChat-2-Max')}`")
